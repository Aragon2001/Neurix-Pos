...
class Purchases_model extends CI_Model
{
        ...
        if ($this->db->insert('purchases', $data)) {
            ...
            foreach ($items as $item) {
                ...
                if($this->db->insert('purchase_items', $item)) {
                    $this->setStoreQuantity($item['product_id'], $data['store_id'], $item['quantity'], $price);
    ...
    public function updatePrice($price, $margin, $cost, $product_id) {
        $this->db->update('products', array('price' => $price, 'margen' => $margin, 'cost' => $cost), array('id' => $product_id));
    }

    public function setStoreQuantity($product_id, $store_id, $quantity, $price) {
        if ($store_qty = $this->getStoreQuantity($product_id, $store_id)) {
            $this->db->update('product_store_qty', array('quantity' => ($store_qty->quantity+$quantity), 'price' => $price), array('product_id' => $product_id, 'store_id' => $store_id));
        } else {
            $this->db->insert('product_store_qty', array('product_id' => $product_id, 'store_id' => $store_id, 'quantity' => $quantity, 'price' => $price));
        }
    }
