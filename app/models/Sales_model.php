...
class Sales_model extends CI_Model
{
    ...
    public function getSaleByIDapartado($id) {
    ...
    public function Anularapartado($id) {
        $q = $this->db->get_where('layaway', array('id' => $id), 1);
        foreach ($q->result() as $apartado) 
        {
            if($apartado->status!='ANULADO')
            {
                $this->db->update('layaway', array('status' => 'ANULADO'), array('id' => $id));
                $pro = $this->db->get_where('layaway_items', array('apartado_id' => $id));
                $proActivo='';
                $proInvent='';
                foreach ($pro->result() as $item) 
                {
                    $proActivo = $this->db->get_where('products', array('id' => $item->product_id), 1);
                    $proInvent = $this->db->get_where('product_store_qty', array('product_id' => $item->product_id), 1);

                    foreach ($proActivo->result() as $proa) 
                    {

                        foreach ($proInvent->result() as $proi) 
                        {

                            if ($proa->type == 'standard') 
                            {
                                $this->db->update('product_store_qty', array('quantity' => ($proi->quantity + $item->quantity)), array('product_id' => $proa->id));
                            } elseif ($proa->type == 'combo') {
                                $combo_items = $this->getComboItemsByPID($proa->id);
                                foreach ($combo_items as $combo_item) 
                                {
                                    ...
                                    if ($cpr->type == 'standard') 
                                    {
                                        ...
                                        $this->db->update('product_store_qty', array('quantity' => ($cpr->quantity + $qty)), array('product_id' => $cpr->id));
                                    }
                                }
                            }
                        }    
                ...
                return array('msg' => 'Apartado numero '.$id.' a sido anulado', 'Status'=>'1');
            }
            else
            {
                return array('msg' => 'ERROR, Apartado numero '.$id.' ya esta anulado', 'Status'=>'2');
            } 
        }    
        return FALSE;
    }
    ...
    public function getSalePaymentsapartado($apartado_id) {
        $this->db->order_by('id', 'asc');
        $q = $this->db->get_where('payments_apartado', array('apartado_id' => $apartado_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getPaymentByIDapartado($id) {
        $q = $this->db->get_where('payments_apartado', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
     ...
     public function getPaymentsAfterApartados($id, $id_apartado){
        $q = $this->db->get_where('payments_apartado', array('id >' => $id, 'apartado_id' => $id_apartado), 1);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function updatePaymentapartado($id, $data = array()) {
        $payment = $this->getPaymentByIDapartado($id);
        if ($payment->paid_by == 'gift_card') {
            $gc = $this->site->getGiftCard($payment->gc_no);
            $this->db->update('gift_cards', array('balance' => ($gc->balance + $payment->amount)), array('card_no' => $payment->gc_no));
        }
        if ($this->db->update('payments_apartado', $data, array('id' => $id))) {
            if ($data['paid_by'] == 'gift_card') {
                $gc = $this->site->getGiftCard($data['gc_no']);
                $this->db->update('gift_cards', array('balance' => ($gc->balance - $data['amount'])), array('card_no' => $data['gc_no']));
            }
            $this->syncSalePaymentsapartado($data['apartado_id']);
            return true;
        }
        return false;
    }
    ...
    public function deletePaymentapartado($id) {
        $payment = $this->getPaymentByIDapartado($id);
        if ($payment->paid_by == 'gift_card') {
            $gc = $this->site->getGiftCard($payment->gc_no);
            $this->db->update('gift_cards', array('balance' => ($gc->balance + $payment->amount)), array('card_no' => $payment->gc_no));
        }
        if ($this->db->delete('payments_apartado', array('id' => $id))) {
            $this->syncSalePaymentsapartado($payment->apartado_id);
            return true;
        }
        return FALSE;
    }

    public function addPaymentapartado($data = array()) {
        if ($this->db->insert('payments_apartado', $data)) {
    ...
    public function syncSalePaymentsapartado($id) {
        $apartado = $this->getSaleByIDapartado($id);
        $payments = $this->getSalePaymentsapartado($id);
        $paid = 0;
        if($payments) {
        ...
        $status = $paid <= 0 ? 'due' : ($apartado->grand_total <= $paid ? 'paid' : 'partial');
        if ($this->db->update('layaway', array('paid' => $paid, 'status' => $status), array('id' => $id))) {
            return true;
        }
    ...
    public function getAllApartadoItems($sale_id) {
    
        $q = $this->db->get_where('layaway_items', array('apartado_id' => $sale_id));
        if($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
