    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <table width="100%" class="stable">
                <tr>
                    <td style="border-bottom: 1px solid var(--nx-border);"><h4><?= lang('cash_in_hand'); ?>:</h4></td>
                ...
                <tr>
                    <td style="border-bottom: 1px solid var(--nx-border);"><h4><?= lang('cash_sale'); ?>:</h4></td>
                    <td style="text-align:right; border-bottom: 1px solid var(--nx-border);"><h4>
                ...
                <tr>
                    <td style="border-bottom: 1px solid var(--nx-border);"><h4><?= lang('ch_sale'); ?>:</h4></td>
                    <td style="text-align:right;border-bottom: 1px solid var(--nx-border);"><h4>
                            <span><?= $this->tec->formatMoney($chsales->paid ? $chsales->paid : '0.00') . ' (' . $this->tec->formatMoney($chsales->total ? $chsales->total : '0.00') . ')'; ?></span>
                        </h4></td>
                </tr>
                <tr>
                    <td style="border-bottom: 1px solid var(--nx-border);"><h4><?= lang('gc_sale'); ?>:</h4></td>
                    <td style="text-align:right;border-bottom: 1px solid var(--nx-border);"><h4>
                            <span><?= $this->tec->formatMoney($gcsales->paid ? $gcsales->paid : '0.00') . ' (' . $this->tec->formatMoney($gcsales->total ? $gcsales->total : '0.00') . ')'; ?></span>
                        </h4></td>
                </tr>
                <tr>
                    <td style="border-bottom: 1px solid <?= (!isset($Settings->stripe)) ? 'var(--nx-border)' : 'var(--nx-border)'; ?>;"><h4><?= lang('cc_sale'); ?>:</h4></td>
                    <td style="text-align:right;border-bottom: 1px solid <?= (!isset($Settings->stripe)) ? 'var(--nx-border)' : 'var(--nx-border)'; ?>;"><h4>
                ...
                <?php if (isset($Settings->stripe)) { ?>
                    <tr>
                        <td style="border-bottom: 1px solid var(--nx-border);"><h4><?= lang('stripe'); ?>:</h4></td>
                        <td style="text-align:right;border-bottom: 1px solid var(--nx-border);"><h4>
                ...
                if ($suspended_bills) {
                    ...
                    foreach ($suspended_bills as $bill) {
                        echo '<tr><td>' . $bill->customer_name . '</td><td>' . $this->tec->hrld($bill->date) . '</td><td class="col-4">' . $bill->hold_ref . '</td><td class="text-right">' . $bill->grand_total . '</td><td class="text-center">'.($Admin ? '<a class="tip" title="' . lang("delete_bill") . '" href="' . site_url('sales/delete_holded/' . $bill->id) . '" onclick="return confirm(\''.lang('alert_x_holded').'\')"><i class="fa fa-trash-o"></i></a>' : '<i class="fa fa-trash-o"></i>').'</td></tr>';
                    }
                    echo '</tbody></table>';
                }
