...
<html lang="en">
    ...
    <main>
        <div id="details" class="clearfix">
            ...
            <div id="invoice">
                <h1>
                 <?php if($hacienda->tipo_doc === "4"){ ?>
                                <b><?= lang("electronic_bill") ?></b><br>
                                <?php }else if($hacienda->tipo_doc === "1"){ ?>
        ...
        <table border="0" cellspacing="0" cellpadding="0">
            <thead>
                <tr>
                    ...
                    <th class="qty"><?= lang('col_unidad_medida'); ?></th>
                    <th class="total"><?= lang('col_total'); ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($rows as $row) {
                    // nx_linea_comprobante() unifica los nombres de columna,
                    // que difieren entre las tablas de lineas de cada documento.
                    $l = nx_linea_comprobante($row); ?>
                    <tr>
                        <td class="no"><?= html_escape($row->product_code) ?></td>
                        <td class="desc"><?= html_escape($row->product_name) ?>
                            (<?= html_escape($l['iva']) ?>)</td>
                        <td class="unit"><?= number_format($l['precio'], $Settings->decimals, $Settings->decimals_sep, $Settings->thousands_sep) ?></td>
                        <td class="qty"><?= $this->tec->formatQuantity($row->quantity) ?></td>
                        <td class="qty"><?= html_escape($l['unidad']) ?></td>
                        <td class="total"><?= number_format($l['subtotal'], $Settings->decimals, $Settings->decimals_sep, $Settings->thousands_sep) ?></td>
                    </tr>
                <?php } ?>
            </tbody>
            <tfoot>
                ...
                <?php if ($inv->PorcentajeExoneracion>0) { ?>
                    <tr>
                        <td colspan="2"></td>
                        <td colspan="2"><?= lang('total_exoneracion'); ?></td>
