                ...
                <div class="table-responsive">
                            <table class="table table-striped table-condensed">
                                ...
                                foreach ($rows as $row) {
                                    ...
                                    echo $this->tec->formatMoney(($row->net_unit_price / $row->quantity) + ($row->item_discount / $row->quantity)) . '</td>';
                                    echo '<td class="text-right">' . $this->tec->formatMoney($row->net_unit_price + $row->item_discount) . '</td></tr>';
                                ...
                                <tr>
                                    ...
                                    <th colspan="2"
                                        class="text-right"><?= $this->tec->formatMoney($inv->total + $inv->product_tax + $inv->product_discount);  ?></th>
                                </tr>
                                <tr>
                                    <th colspan="2"><?= lang("total"); ?> sin impuesto</th>
                                    <th colspan="2"
                                        class="text-right"><?= $this->tec->formatMoney($inv->total);  ?></th>
                                </tr>
                                <?php
//                                dd($inv);
                                if ($inv->total_tax != 0) {
                            ...
                            <hr style="border-top: 1px solid var(--nx-border) !important; "/>
                            <p style="font-size: 14px !important;">
                                <br>
                                <b><?= lang('reason_credit_note') ?>:</b><?php if($inv->hold_ref != ""){
                                    echo  " ".$inv->hold_ref;
                                }else{
                                    echo  " ".$credit_note->hold_ref;
                                } ?><br>
                                <b><?= lang('referenced_document') ?>:</b> <?= lang("electronic_bill") ?><br>
                                <b><?= lang('consecutive_referenced_document') ?>:</b> <?= $hacienda->consecutivo ?><br>
                                <b><?= lang('key_referenced_document') ?>:</b> <?= $hacienda->clave ?>
                            </p>
                            <hr style="border-top: 1px solid var(--nx-border) !important; "/>
                            ...
                            <?php if (!empty($store->receipt_footer)) { ?>
                                <div class="well well-sm" style="margin-top:10px;">
                                    ...
                                    <br/>
                                    <div style="text-align: center;"><?= $Settings->footer_hacienda_nc ?>
                                    </div>
                        ...
                        bootbox.prompt({
                            ...
                            callback: function (email) {
                                if (email != null) {
                                    $.ajax({
                                        type: "post",
                                        url: "<?= site_url('pos/email_receipt_credit') ?>",
