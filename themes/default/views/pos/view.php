...
//                                    echo '<img src="' . base_url('uploads/' . $store->logo) . '" alt="' . $store->name . '">';
                            ...
                            <p>
                                ...
                                <?= lang('Consecutivo N°') . ': ' . $hacienda->consecutivo; ?><br>
                                <?= lang("customer") . ': ' . $inv->customer_name; ?> <br>

                                <? if($customer->cf1 == "01"){ ?>
                                    <?= lang("Cedula Identidad") . ': ' . $customer->cf2; ?> <br>
                                <? }else if($customer->cf1 == "02"){ ?>
                                    <?= lang("Cedula Juridica") . ': ' . $customer->cf2; ?> <br>
                                <? }else if($customer->cf1 == "03"){ ?>
                                    <?= lang("Dimex") . ': ' . $customer->cf2; ?> <br>
                                <? }else if($customer->cf1 == "04"){ ?>
                                    <?= lang("NITE") . ': ' . $customer->cf2; ?> <br>
                                <? } ?>
                ...
                <div class="table-responsive">
                            <table class="table table-striped table-condensed">
                                ...
                                <tr>
                                    <th class="text-center"
                                        style="width: 50%; border-bottom: 2px solid var(--nx-border);"><?= lang('description'); ?></th>
                                    <th class="text-center"
                                        style="width: 12%; border-bottom: 2px solid var(--nx-border);"><?= lang('quantity'); ?></th>
                                    <th class="text-center"
                                        style="width: 24%; border-bottom: 2px solid var(--nx-border);"><?= lang('price'); ?></th>
                                    <th class="text-center"
                                        style="width: 26%; border-bottom: 2px solid var(--nx-border);"><?= lang('subtotal'); ?></th>
                                </tr>
                                </thead>
                                <tbody>
                                <?php
                                ...
                                foreach ($rows as $row) {
                                    ...
                                    if($row->tax > 0){
                                       $row->net_unit_price = $row->net_unit_price + ($row->net_unit_price * ($row->tax / 100)) ;
                                    }
                                    echo '<tr><td>' . $row->product_name . ' ' . $tt . ' </td>';
                                    echo '<td style="text-align:center;">' . $this->tec->formatQuantity($row->quantity) .'('.$row->unit_of_measurement .')'. '</td>';
                                    echo '<td class="text-right">';
                                    echo number_format($row->net_unit_price , $Settings->decimals, $Settings->decimals_sep, $Settings->thousands_sep) . '</td>';
                                    echo '<td class="text-right">' . number_format($row->subtotal, $Settings->decimals, $Settings->decimals_sep, $Settings->thousands_sep) . '</td></tr>';
                                ...
                                <tfoot>
                                <tr>
                                    <th colspan="2"><?= lang("total"); ?> con impuesto</th>
                                    <th colspan="2"
                                        class="text-right"><?= $this->tec->formatMoney($inv->total + $inv->product_tax + $inv->product_discount + $inv->MontoExoneracion);  ?></th>
                                </tr>
                                <tr>
                                    <th colspan="2"><?= lang("total"); ?> sin impuesto</th>
                                    <th colspan="2"
                                        class="text-right"><?= $this->tec->formatMoney($inv->total+$inv->MontoExoneracion);  ?></th>
                                </tr>
                                <?php
                                ...
                                if ($inv->total_tax != 0 ) {
                                    if($inv->MontoExoneracion){
                                        $montoMenosExo = $inv->total_tax - $inv->MontoExoneracion ;
                                        if($inv->PorcentajeExoneracion == 100){
                                            $montoMenosExo = 0;
                                        }
                                        echo '<tr><th colspan="2">' . lang("order_tax") . '</th><th colspan="2" class="text-right">' . $this->tec->formatMoney($montoMenosExo)  . '</th></tr>';
                                    }else{
                                        echo '<tr><th colspan="2">' . lang("order_tax") . '</th><th colspan="2" class="text-right">' . $this->tec->formatMoney($inv->total_tax) . '</th></tr>';
                                    }
                                }
                            ...
                            <?php if (!empty($store->receipt_footer)) { ?>
                                <div class="well well-sm" style="margin-top:10px;">
                                    ...
                                    <br/>
                                    <div style="text-align: center;"><?= $Settings->footer_hacienda_fe ?>
                                    </div>

                                    <br/>
									 <? if($Settings->fe == 1){ ?>
                                    <div style="text-align: center; width: 100%; font-size: 14px !important;"><b><?= lang('electronic_voucher_key') ?>: <?= $hacienda->clave; ?></b></div>
									<? } ?>
                                    <div style="text-align: center;"><br/><?= lang('internal_id') ?>
