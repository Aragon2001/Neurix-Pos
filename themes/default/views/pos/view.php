    ...
    <div class="modal-content">
        <div class="modal-body">
            ...
            <?php if (!$modal) {
                ...
                <div class="pagebar no-print">
                    <div class="pagebar-in">
                        <a class="pb-back" id="volver-pos" href="<?= site_url('pos'); ?>">
                        ...
                        <div class="pb-actions">
                            <button onclick="return imprimirTiquete()" class="nx-btn nx-btn-primary">
                                <i class="fa fa-print"></i> <span class="lbl"><?= lang("print"); ?></span>
                            </button>
                            <a class="nx-btn nx-btn-ghost" href="<?= site_url('pos/pdf/' . $inv->id); ?>">
                                <i class="fa fa-file-pdf-o"></i> <span class="lbl">PDF</span>
                            </a>
            ...
            <div id="wrapper">
                        ...
                        <div id="receipt-data">
                                ...
                                <tr>
                                    <td>
                                        ...
                                        <?php if (!empty($customer->phone)) { ?>
                                            <div class="inv-party-line"><?= html_escape($customer->phone) ?></div>
                                        <?php } ?>
                                    </td>
                                    <td class="right">
                                        ...
                                        <table class="inv-kv">
                                            ...
                                            <?php } ?>
                                            <tr>
                                                <td class="k"><?= lang('status') ?></td>
                                                <td class="v"><?= lang($inv->payment_status) ?></td>
                                            </tr>
                                    ...
                                    <td class="box">
                                        <table class="inv-tot">
                                            <tr>
                                                ...
                                                <td class="v"><?= $this->tec->formatMoney($inv->total + ($inv->product_tax ?? 0) + ($inv->product_discount ?? 0) + $inv->MontoExoneracion) ?></td>
                                            </tr>
                                            <tr>
                                                <td class="k"><?= lang('total') ?> sin impuesto</td>
                                                <td class="v"><?= $this->tec->formatMoney($inv->total + $inv->MontoExoneracion) ?></td>
                                            </tr>
                                            ...
                                            if ($inv->total_tax != 0) {
                                                if ($inv->MontoExoneracion) {
                                                    ...
                                                    echo '<tr><td class="k">' . lang('order_tax') . '</td><td class="v">' . $this->tec->formatMoney($montoMenosExo) . '</td></tr>';
                                                } else {
                                            ...
                                            if ($inv->paid < $round_total) {
                                                echo '<tr class="sep"><td class="k">' . lang('paid_amount') . '</td><td class="v">' . $this->tec->formatMoney($inv->paid) . '</td></tr>';
                                                echo '<tr class="due"><td class="k">' . lang('due_amount') . '</td><td class="v">' . $this->tec->formatMoney($inv->grand_total - $inv->paid) . '</td></tr>';
                                            }
                                            ?>
                                        </table>
                                    </td>
                            ...
                            <?php if ($payments) { ?>
                                <table class="inv-pay">
                                    <thead>
                                    <tr>
                                        <th style="width:34%;"><?= lang('paid_by') ?></th>
                                        <th style="width:33%;"><?= lang('reference') ?></th>
                                        <th style="width:33%;" class="num"><?= lang('amount') ?></th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    <?php foreach ($payments as $payment) {
                                        ...
                                        if ($payment->paid_by == 'cash' && $payment->pos_paid != '') {
                                            $monto = $payment->pos_paid == 0 ? $payment->amount : $payment->pos_paid;
                                            $vuelto = $payment->pos_balance > 0 ? $this->tec->formatMoney($payment->pos_balance) : 0;
                                            $ref = lang('change') . ': ' . $vuelto;
                                        } elseif (($payment->paid_by == 'CC' || $payment->paid_by == 'ppp' || $payment->paid_by == 'stripe') && $payment->cc_no) {
                                            $monto = $payment->pos_paid;
                                            $ref = 'xxxx xxxx xxxx ' . substr($payment->cc_no, -4);
                                            if ($payment->cc_holder) { $ref .= ' · ' . $payment->cc_holder; }
                                        } elseif ($payment->paid_by == 'Cheque' || ($payment->paid_by == 'cheque' && $payment->cheque_no)) {
                                            $monto = $payment->pos_paid;
                                            $ref = lang('cheque_no') . ': ' . $payment->cheque_no;
                                        } elseif ($payment->paid_by == 'gift_card' && $payment->pos_paid) {
                                            $monto = $payment->pos_paid;
                                            $ref = lang('no') . ': ' . $payment->gc_no;
                                        } elseif ($payment->paid_by == 'other' && $payment->amount) {
                                            $monto = $payment->pos_paid == 0 ? $payment->amount : $payment->pos_paid;
                                            $ref = $payment->note ? $payment->note : '';
                                        }
                                        ?>
                                        <tr>
                                            <td><?= lang($payment->paid_by) ?></td>
                                            <td><?= html_escape($ref) ?></td>
                                            <td class="num"><?= $this->tec->formatMoney($monto) ?></td>
                                        </tr>
                                    <?php } ?>
                                    </tbody>
                                </table>
                ...
                <?php } elseif (isset($hacienda) && $hacienda && $hacienda->estatus_hacienda === 'aceptado') { ?>
                    <div class="pb-extra no-print">
                ...
                if (enviar) {
                    enviar.addEventListener('click', function () {
                        ...
                        destinos.forEach(function (correo) {
                            ...
                            fetch('<?= site_url('pos/email_receipt') ?>', {
                                method: 'POST',
                                body: cuerpo,
