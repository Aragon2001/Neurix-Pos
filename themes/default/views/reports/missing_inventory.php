...
document.addEventListener('DOMContentLoaded', function () {
    var t = new NxTable({
        el: '#nxtList',
        url: '<?= site_url('reports/get_missing_inventory/' . $v); ?>',
        csrf: { name: '<?= $this->security->get_csrf_token_name(); ?>', hash: '<?= $this->security->get_csrf_hash(); ?>' },
        minWidth: '700px',
        unit: '<?= lang('products'); ?>'.toLowerCase(),
        exportName: 'inventario_faltante',
        search: ['producto', 'product_id'],
        columns: [
            { key: 'product_id', label: '<?= lang('code'); ?>', sortable: 'str', render: function (r) { return '<span class="nxt-code">' + NxTable.esc(r.product_id) + '</span>'; } },
            { key: 'producto', label: '<?= lang('product'); ?>', sortable: 'str', render: function (r) { return '<span class="nxt-ent-name">' + NxTable.esc(r.producto) + '</span>'; } },
            { key: 'faltante', label: '<?= lang('faltante'); ?>', className: 'num', sortable: 'num', render: function (r) {
                return NxTable.badge(NxTable.qty(r.faltante), 'err');
            }, exportValue: function (r) { return r.faltante; } },
            { key: 'emitido', label: '<?= lang('date'); ?>', sortable: 'str', render: function (r) { return '<span class="nxt-dim-mono">' + NxTable.esc(r.emitido) + '</span>'; } }
        ],
        i18n: {
