...
/* Columnas del CSV: las tres primeras son obligatorias. */
$columnas = array(
    'code'          => TRUE,
    'name'          => TRUE,
    'cabys'         => TRUE,
...
$modos = array(
    'crear'      => array(lang('import_modo_crear'),      '<path d="M12 5l0 14"/><path d="M5 12l14 0"/>'),
    'actualizar' => array(lang('import_modo_actualizar'), '<path d="M20 11a8.1 8.1 0 0 0 -15.5 -2m-.5 -4v4h4"/><path d="M4 13a8.1 8.1 0 0 0 15.5 2m.5 4v-4h-4"/>'),
    'ambos'      => array(lang('import_modo_ambos'),      '<path d="M4 6h16"/><path d="M4 12h16"/><path d="M4 18h16"/>'),
);
...
<div class="nxt-head">
    <div class="nxt-title">
        ...
        <small><?= lang('import_ayuda'); ?></small>
    </div>
    <div class="nxt-head-actions">
        <a class="nxt-btn nxt-btn-ghost" href="<?= base_url('uploads/csv/sample_products.csv'); ?>" download>
...
<div class="imp-page">
    ...
    <div class="nxf-card">
        ...
        <div class="nxf-card-body">
            <?= form_open_multipart('products/revisar_import', array('id' => 'impForm')); ?>
            ...
            <div class="nxf-hint" style="margin-top:8px;"><?= lang('import_columnas_ayuda'); ?></div>
        ...
        <div class="nxf-card-body">
            ...
            <?php if ($n_err) { ?>
                <div class="nxf-note nxf-note-err" style="margin-top:16px;">
                    ...
                    <?= form_open('products/descargar_rechazos_import', array('class' => 'imp-form-inline')); ?>
                    ...
                    <table class="imp-tabla">
                        <thead>
                            <tr>
                                ...
                                <th><?= lang('codigo_cabys'); ?></th>
                                <th class="num"><?= lang('product_cost'); ?></th>
                                <th class="num"><?= lang('product_price'); ?></th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach (array('crear' => 'ok', 'actualizar' => 'info') as $clave => $tono) {
                                foreach ($rev[$clave] as $f) { ?>
                                    <tr>
                                        ...
                                        <td class="mono"><?= html_escape($f['cabys']); ?></td>
                                        <td class="num mono"><?= html_escape(isset($f['cost']) ? $f['cost'] : '—'); ?></td>
                                        <td class="num mono"><?= html_escape(isset($f['price']) ? $f['price'] : '—'); ?></td>
                                    </tr>
                                <?php }
                            } ?>
                        </tbody>
            ...
            <?php if ($n_ok) { ?>
                <?= form_open('products/confirmar_import'); ?>
...
<style>
    ...
    .imp-form-inline{margin-left:auto}
    ...
    .imp-tabla{width:100%;border-collapse:collapse;font-size:12.5px;min-width:720px}
    .imp-tabla thead th{
