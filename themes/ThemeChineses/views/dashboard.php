...
$('#chart2').highcharts({
    ...
    series: [{
        ...
        foreach($topProducts as $tp) {
            echo "['".str_replace("'","",strval($tp->product_name))." (".strval($tp->product_code)." )', ".$tp->quantity."],";
...
$('#chart3').highcharts({
    ...
    series: [{
        type: 'pie',
        name: '<?=$this->lang->line('cost_price')?>',
        data: [
        <?php

        foreach($costAndPriceInv as $tp) {
            echo "['(".$this->lang->line('cost').")', ".$tp->cost."],['(".$this->lang->line('price').")' ,".$tp->price." ]";
        } ?>
        ]
    }]
});
...
<!--                    <a class="btn btn-app" href="<?= site_url('gift_cards'); ?>">
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="box box-primary">
                        <div class="box-header">
                            <h3 class="box-title"><?= lang('cost_inv').' ('.date('F Y').')'; ?></h3>
                        </div>
                        <div class="box-body">
